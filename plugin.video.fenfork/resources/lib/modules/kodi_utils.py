...
addon_object = xbmcaddon.Addon('plugin.video.fenfork')
...
def build_url(url_params):
	return 'plugin://plugin.video.fenfork/?%s' % urlencode(url_params)
...
def addon(addon_id='plugin.video.fenfork'):
...
def run_addon(addon='plugin.video.fenfork', block=False):
...
def disable_enable_addon(addon_name='plugin.video.fenfork'):
	try:
...
def update_kodi_addons_db(addon_name='plugin.video.fenfork'):
	import time
	import sqlite3 as database
	try:
...
def toggle_language_invoker():
	close_all_dialog()
	sleep(100)
	addon_xml = translate_path('special://home/addons/plugin.video.fenfork/addon.xml')
...
def open_settings(query, addon='plugin.video.fenfork'):
	hide_busy_dialog()
	if query:
		try:
