...
def call_trakt(path, params={}, data=None, is_delete=False, with_auth=True, method=None, pagination=False, page_no=1):
	def send_query():
		...
		if with_auth:
			try:
				try: expires_at = float(get_setting('fenfork.trakt.expires'))
				except: expires_at = 0.0
				if time.time() > expires_at: trakt_refresh_token()
			except: pass
			token = get_setting('fenfork.trakt.token')
	...
	if status_code in (303, 403):
		notification('Trakt request rejected (%s). Please re-authorize Trakt.' % status_code, 4000)
		return None
	if status_code == 401:
		...
		if 'Retry-After' in headers:
			...
			sleep(1000 * max(retry_after, 1))
			response = send_query()
			try: status_code = response.status_code
			except: return None
	if status_code >= 400:
		logger('Trakt Error', 'HTTP %s (%s)' % (status_code, path))
		return None
	...
	try:
		...
		try:
			...
			while not progressDialog.iscanceled() and time_passed < expires_in:
				...
				elif status_code in (303, 403):
					notification('Trakt authorization blocked (%s). Please re-authorize and try again.' % status_code, 5000)
					break
				else: break
		except: pass
...
def trakt_refresh_token():
	data = {        
		...
		'grant_type': 'refresh_token', 'refresh_token': get_setting('fenfork.trakt.refresh')}
	response = call_trakt("oauth/token", data=data, with_auth=False)
	if response:
		manage_settings_reset()
		set_setting('fenfork.trakt.token', response["access_token"])
		set_setting('fenfork.trakt.refresh', response["refresh_token"])
		set_setting('fenfork.trakt.expires', str(time.time() + 86400))
...
def trakt_authenticate(dummy=''):
	...
	if token:
		manage_settings_reset()
		set_setting('fenfork.trakt.token', token["access_token"])
		set_setting('fenfork.trakt.refresh', token["refresh_token"])
		set_setting('fenfork.trakt.expires', str(time.time() + 7776000))
		set_setting('fenfork.trakt.indicators_active', 'true')
		set_setting('fenfork.watched_indicators', '1')
		manage_settings_reset(True)
		sleep(1000)
		try:
			user = call_trakt('/users/me')
			set_setting('fenfork.trakt.user', str(user['username']))
		except: pass
...
def trakt_revoke_authentication(dummy=''):
	data = {'token': get_setting('fenfork.trakt.token'), 'client_id': CLIENT_ID, 'client_secret': CLIENT_SECRET}
	response = call_trakt("oauth/revoke", data=data, with_auth=False)
	manage_settings_reset()
	set_setting('fenfork.trakt.user', '')
	set_setting('fenfork.trakt.expires', '')
	set_setting('fenfork.trakt.token', '')
	set_setting('fenfork.trakt.refresh', '')
	set_setting('fenfork.trakt.indicators_active', 'false')
	set_setting('fenfork.watched_indicators', '0')
	manage_settings_reset(True)
...
def trakt_calendar_days(recently_aired, current_date):
	if recently_aired: start, finish = (current_date - timedelta(days=14)).strftime('%Y-%m-%d'), '14'
		...
		previous_days = int(get_setting('fenfork.trakt.calendar_previous_days', '3'))
		future_days = int(get_setting('fenfork.trakt.calendar_future_days', '7'))
...
def trakt_sync_activities(force_update=False):
	...
	if not get_setting('fenfork.trakt.user', '') and not force_update: return 'no account'
